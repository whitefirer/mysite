---
title: "给 DeepSeek Harness 桌宠装耳朵：浏览器里跑中文 KWS 的全记录"
subtitle: "dsh 插件的四代耳朵演进：从 MFCC 模板匹配到 sherpa-onnx wasm 自编译，17MB 不走 CDN、512MB 内存砍到 32MB、Worker 空闲真释放，外加四词交叉验证零误报"
description: "dsh 桌宠牛来的「循环喊到互动停止」需要一双耳朵：喊一声「牛来」它就停。记录这双耳朵的四代演进——零模型 MFCC+DTW 模板匹配的判别力天花板、sherpa-onnx KWS wasm 自编译、插件 host 半开路由同源分发 17MB、INITIAL_MEMORY 从 512MB 实测定稿 32MB、Web Worker 空闲 10s terminate 真释放、四指令词交叉验证零串词零误报，以及真机「音量小识别不到」的最后一坑。"
