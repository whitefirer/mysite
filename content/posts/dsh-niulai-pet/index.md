---
title: "任务一完成，它就蹦出来喊「妈」——给 DeepSeek Harness 养一头牛来"
subtitle: "一个纯客户端插件的诞生：视频抠图修蹄子、鬼畜音频基频破案、嘴型时间线、WAAPI 动画三连坑，外加三只手绘小动物"
...
tags: ["tech", "ai", "deepseek", "harness", "plugin", "desktop-pet", "牛来"]
categories: ["Tech"]
...
不认识这头牛的同学补一下背景：《牛来》是最近很火的一部 AI 生成短片，主角是一头丑得很有特色的黄牛。它在 B 站的二创鬼畜里被加速着喊了几十声"妈"，越到后面音调越高，成了名场面。我把它截下来养进了 DeepSeek Harness（dsh）的 web 界面里——就是那个我最近连着折腾了好几篇的 agent 工作台。
...
- **五个皮肤**：牛来（电影截帧）、小黄（幼年版，无角更黄亮）、奶牛、熊猫、蓝鲸——后三只没有现成素材，是我用 SVG 一只一只手绘的，蓝鲸特意调了 DeepSeek 蓝、加了虎鲸的白眼斑。
- **签名动作**：牛来连跳、小黄和熊猫翻滚、奶牛摇摆、蓝鲸弧线跃出水面（到弧顶还会喷水）。动作可以绑到事件上，也能设成随机。
- **叫声**：牛来和小黄用原声（从鬼畜视频里截出来再降噪），奶牛/熊猫/蓝鲸的叫声是 WebAudio 现场合成的——哞、吱、鲸鸣，零素材零版权。
...
先说个事：《牛来》这片子没有海报，我抠图用的那张"蓝天下站着一头牛"的定格帧，**本身也是 AI 生成的**。所以这事的全产业链是：AI 画的牛 → AI（我）抠出来 → 住进 AI 工作台 → 文章也是 AI 参与写的。谁也不欠谁。
...
喊声选自那个 70 秒的"喊妈妈"鬼畜合集。截出来的四段候选里有两段怎么听都不像牛来的声音——不是耳朵好，是我跑了自相关基频分析：真奶声基频 800-889Hz，那两段只有 516-552Hz。**鬼畜视频越到后面越快，音调跟着速度一起飞了**，低近一半的基频就是加速段的铁证。
...
三只手绘动物没有真声可截，干脆全部 WebAudio 合成：哞 = 锯齿波下滑 + 低频震颤 + 低通；鲸鸣 = 正弦长音先扬后抑 + 慢颤音；吱声 = 两声三角波短促上挑。总共不到 60 行代码，不用下载任何音频，也没有任何版权问题。
...
素材直接入库，装上即玩——反正连源图都是 AI 生成的，牛来本来就是这个 AI meme 生态的二创常客，没什么可端着的。奶牛/熊猫/蓝鲸三只手绘的 SVG 源文件也在仓库 `tools/drawn/` 里，随便拿去用。
...
下一步：自定义皮肤（导入自己的图和声音）、试试发插件市场。仓库在 [github.com/whitefirer/dsh-niulai-pet](https://github.com/whitefirer/dsh-niulai-pet)，素材全入库，装上即玩。
