...
不认识这头牛的同学补一下背景：《牛来》是最近很火的一部短片，主角是一头丑得很有特色的黄牛，喊"妈"是它的名场面。这片子的二创生态相当繁荣，各种鬼畜剪辑满天飞——把喊声加速变调、越到后面音调越高的那种。我把它养进了 DeepSeek Harness（dsh）的 web 界面里——就是那个我最近连着折腾了好几篇的 agent 工作台。
...
素材全部入库，装上即玩。奶牛/熊猫/蓝鲸三只手绘的 SVG 源文件在仓库 `tools/drawn/` 里，随便拿去用。
